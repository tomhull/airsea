...
test = data.frame(timePeriod = 0.01, # tiny timestep to neglect the calculation of out-gassing
                   ...
                   u0 = 1E-3, u1 = 1E-3, # tiny wind speed to further neglect out-gassing
                   h0 = 1, h1 = 1, # shallow unchanging 1m mld
...
This should be equivilent to just the gas exchange:

```{r net_production_presuper_zerochange_GE}
kw("O2", 10, 7, 35) * (Csat(10, 35) - Cbubble)*10*60
```
...
```{r entrainment_concept}
  # conceptually:
  # 10 1m-3 bins with concentration at saturation
v1 = 10
C1 = Csat(10, 35) * (1 + bubbleSat("O2", 0.1) )
  # entrain 1m-3 of water with concentration of -10
v2 = 1
C2 = C1 - 10
...
test = data.frame(timePeriod = 1000,
                   C0 = Csat(10, 35), C1 = C3 , # as above
                   Cb0 = 200, Cb1 = 200 , # as above
                   T0 = 10, T1 = 10, # stable temperature
                   S0 = 35, S1 = 35, # stable salinity
                   u0 = 1, u1 = 1, # stable wind (although this won't be a factor)
                   h0 = 3, h1 = 4, # shallow unchanging 1m mld
                   Pslp0 = 1015.25, Pslp1 = 1013.25 # standard  sea level pressure (again not used)
                   )
print(O2NCP.mean(test, bubbleoff = T)) # expect 0
```
...
loop_test <- function(test, NCP, accuracy = 1){
  ...
  # entrainment
  if("Cb0" %in% colnames(test)){
    print("Cb found, entrainment calculated")
...
test = loop_test(test, 0) # expect C1 to be 281.1138
